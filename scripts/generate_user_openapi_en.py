...
待创建委托，1 至 5 项|||Orders to create, from 1 to 5 items
待取消的订单列表|||Orders to cancel
待取消订单，1 至 10 项|||Orders to cancel, from 1 to 10 items
...
返回指定结果 Token 的买卖盘订单簿。|||Return the bid and ask order book for an outcome token.
...
为当前登录账户创建委托。|||Create an order for the authenticated account.
...
列表为空、超过 10 条或订单参数无效|||The list is empty, exceeds 10 items, or contains invalid order parameters
列表为空、超过 5 条或委托参数无效|||The list is empty, exceeds 5 items, or contains invalid order parameters
买卖价差|||Bid-ask spread
...
符合条件的当前委托总数|||Total number of matching open orders
...
同步创建 1 至 5 笔委托并返回每笔处理结果。整批订单共用 Edge 生成的订单时间；GTD 的 expiration 必填且使用 Unix 毫秒，并且必须晚于服务端订单时间。|||Synchronously create 1 to 5 orders and return the result for each one. All orders in the batch share the order timestamp generated by Edge. For a GTD order, `expiration` is required, must use Unix milliseconds, and must be later than the server-generated order time.
...
取消当前登录账户下的指定委托。|||Cancel the specified order owned by the authenticated account.
取消委托|||Cancel order
...
锁定余额|||Locked balance
条件 ID|||Condition ID
同步创建 1 至 5 笔委托并返回每笔处理结果。每项 timestamp 使用 Unix 毫秒且每次请求都应取当前时间；expiration 支持 Unix 秒或毫秒，传秒时服务端自动转换。|||Synchronously submit 1 to 5 orders and return the result for each item. Set each `timestamp` to the current Unix time in milliseconds. `expiration` accepts Unix seconds or milliseconds; the service converts seconds to milliseconds.
同步取消 1 至 10 笔委托。每项 tokenId 可省略，但建议传入以完成事件权限和市场生命周期校验。|||Synchronously cancel 1 to 10 orders. `tokenId` is optional, but providing it enables event-permission and market-lifecycle validation.
图标 URL|||Icon URL
图片 URL|||Image URL
